...
get_archpkg (){
  ...
  wget https://aur.archlinux.org/packages/en/light-capricoin-git/light-capricoin-git.tar.gz
  tar -xpzvf light-capricoin-git.tar.gz
  sed -e 's/_gitbranch\=.*/_gitbranch='${archbranch}'/g' light-capricoin-git/PKGBUILD > light-capricoin-git/PKGBUILD.new
  mv light-capricoin-git/PKGBUILD.new light-capricoin-git/PKGBUILD
  rm light-capricoin-git.tar.gz
  popd
}
prepare_repo(){
...
buildRelease(){
  ...
  # echo "Making locales" 
  # $DOCKERBIN run --rm -it --privileged -e MKPKG_VER=${VERSION} -v $(pwd)/helpers:/root  -v $(pwd)/repo:/root/repo  -v $(pwd)/source:/opt/wine-light/drive_c/light-capricoin/ -v $(pwd):/root/light-capricoin-release capricoin/light-capricoin-release:${VERSION} /bin/bash
...
build_Windows(){
   echo "Making Windows EXEs for $VERSION" \
    ...
    && ls -la $(pwd)/helpers/release-packages/Windows/Light-Capricoin-${VERSION}-Windows-setup.exe \
...
buildImage(){
  echo "Building image"
  case "${1}" in 
  winbuild) $DOCKERBIN build -t capricoin/light-capricoin-winbuild:${VERSION} .
         ;;
   release) $DOCKERBIN build -f Dockerfile-release -t  capricoin/light-capricoin-release:${VERSION} .
         ;;
  esac
}
...
prepareFile(){
  echo "Preparing file for Light-Capricoin version $VERSION"
  if [ -e "$TARGETPATH" ]; then
    echo "Version tar already downloaded."
  else
   wget https://github.com/capricoin/light-capricoin/archive/v${VERSION}.zip -O $TARGETPATH
  fi
