...
if [[ $# -gt 0 ]]; then
  ...
  export FILENAME=Light-Capricoin-$VERSION.zip
  echo "export FILENAME=Light-Capricoin-$VERSION.zip" >> build-config.sh
  export TARGETPATH=$(pwd)/source/$FILENAME
  echo "export TARGETPATH=$(pwd)/source/$FILENAME" >> build-config.sh
  export TARGETFOLDER=$(pwd)/source/Light-Capricoin-$VERSION
  echo "export TARGETFOLDER=$(pwd)/source/Light-Capricoin-$VERSION" >> build-config.sh
  echo "Building Light-Capricoin $VERSION from $FILENAME"
else
